...
const MyReviewCard = ({ review, myFavoriteDelete }) => {
	const handleRemoveReview = (id) => {
		DeleteModal(function () {
			api.delete(`http://localhost:3000/api/reviews/${id}`)
				.then((response) => {
					if (myFavoriteDelete) {
						myFavoriteDelete(id);
					}
	...
	return (
		<div key={review.id} className="card overflow-hidden border-gray-200 border-2 shadow-md">
			<div className="md:flex">
				{/* Image */}
				<div className="md:w-40 ">
					<img src={review.image || '/placeholder.svg'} alt={review.foodName} className="h-40 w-full object-cover" />
				</div>

				{/* Content */}
				<div className="p-4 flex-1 flex flex-col">
					<div>
						<Link to={`/review/${review.id}`} className="block hover:text-primary mb-1">
							<h3 className="font-bold text-lg text-text-primary">{review.foodName}</h3>
						</Link>
					...
					<div className="mt-2 mb-4">
						<div className="flex gap-1 mb-1">
							{Array.from({ length: 5 }).map((_, i) => (
								<span key={i} className={`text-sm ${i < Math.floor(review.rating) ? 'text-accent' : 'text-gray-300'}`}>
									★
								</span>
							))}
						</div>
						<p className="text-xs text-text-secondary">{formatDateForWeb(review.createdAt)}</p>
					</div>
